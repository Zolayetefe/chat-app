...
function ChatLayout() {
  ...
  if (!user) {
    // A slightly more descriptive unauthenticated state
    return (
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 antialiased">
      ...
      {/* Sidebar - Desktop */}
      {/* Use a fixed sidebar with a subtle border and more elevated shadow */}
      <aside className="w-64 bg-white border-r border-gray-200 shadow-xl hidden md:flex flex-col fixed h-full z-20">
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          ...
          {/* Add more nav items (e.g., Groups, Settings) for a complete professional app */}
        </nav>
      ...
      <aside className={`fixed top-0 left-0 w-64 h-full bg-white border-r border-gray-200 shadow-2xl z-40 transform transition-transform duration-300 md:hidden flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* Content duplicated from desktop sidebar for mobile */}
        <div className="p-6 border-b border-gray-100">
          <h1 className="text-2xl font-bold text-gray-800">
            <span className="text-blue-600">Chat</span>Pro
          </h1>
      ...
      {/* This replaces the original Mobile Header and serves as a Top Bar for continuity */}
      <header className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-200 shadow-md z-10 md:ml-64">
        <div className="flex items-center justify-between h-full px-4">
          ...
          {/* Quick links/actions can go here on the right, but we'll stick to a placeholder */}
          <div className="flex items-center space-x-4">
            {/* Desktop Logout Button - redundant with sidebar but good for mobile view */}
            <button
              onClick={handleLogout}
      ...
      {/* Main Content Area */}
      {/* We need margin-left for desktop (md:ml-64) and padding-top for the fixed header (pt-16) */}
